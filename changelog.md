...
- Now the method `to_pdf` has a new boolean parameter `do_legend` to add the legend figure at the end of the pdf.
- Now the function `save_figs_in_pdf` has a new parameter `do_padding` to choose to add a padding or not in the pdf.
- Now the method `to_zip` adds the legend to the zip file.
- The color of the mutual information figure is now closer to the color of the same figure in the app.
- Fixed a bug in the mutual information figure where the xlabel and ylabel were permuted.
- Renamed the function `generate_insightsolver_banner` to `generate_insightsolver_img_banner` to be more corehent with the naming of the function that generates the legend.
- New function `generate_insightsolver_fig_banner`.
- Now the method `to_pdf` exploits `generate_insightsolver_fig_banner`.
- Now the method `to_zip` exploits `generate_insightsolver_fig_banner`.
- Now the distributions generated by `show_feature_distributions_of_S_feature` have the InsightSolver blue with `alpha=1.0`.
...
- Changed the name of the figures generated by the method `to_zip`.
