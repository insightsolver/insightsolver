...
def generate_insightsolver_banner_old(
    solver,
    i:int,
    ...
    coverage_absolute_text = rule_i['m']
    ...
    if loss==None:
        ...
        # Draw the relative coverage
        coverage_relative_text     = str(round(coverage_relative*100,2))+'%'
        coverage_relative_position = (1080, 20)
        draw.text(coverage_relative_position, coverage_relative_text, font_size=font_size, fill="black")
        # Draw the absolute coverage
        coverage_absolute_text     = str(coverage_absolute)
        coverage_absolute_position = (1310, 20)
        draw.text(coverage_absolute_position, coverage_absolute_text, font_size=font_size, fill="black")
        # Draw the cohen d (NEW FEATURE)
        cohen_d_text     = f"{cohen_d:.2f}"
        cohen_d_position = ... # NEW TEXT to add
        ...
        # Draw the relative coverage
        coverage_relative_text     = str(round(coverage_relative*100,2))+'%'
        coverage_relative_position = (950, 22)
        draw.text(coverage_relative_position, coverage_relative_text, font_size=font_size, fill="black")
        # Draw the absolute coverage
        coverage_absolute_text     = str(coverage_absolute)
        coverage_absolute_position = (1160, 22)
        draw.text(coverage_absolute_position, coverage_absolute_text, font_size=font_size, fill="black")
        # Draw the cohen d (NEW FEATURE)
        cohen_d_text     = f"{cohen_d:.2f}"
        cohen_d_position = ... # NEW TEXT to add
